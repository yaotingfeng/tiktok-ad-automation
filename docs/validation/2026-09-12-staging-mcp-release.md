...
本次数据库、Redis、私有配置备份及旧 release 保留已执行，但未生成独立项目源码/构建产物归档。发布后已在 AGENTS.md、通用部署手册及测试环境手册补充每次发版完整归档、配置核对和恢复验证的强制要求；现有服务器备份脚本尚未自动覆盖项目归档，后续部署须补执行手册中的人工步骤。本次文档修订没有修改服务器脚本或补造历史备份。
