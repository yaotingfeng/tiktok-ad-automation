...
- 用户随后明确要求重试剩余四条广告，并接受在原组内进行受控补建。范围固定为新骏伯 BC `7683817908149272592`、R627 与 6X3F 各两条原 UNKNOWN AD；创建前逐组完整回读，原广告名与 `-rebuild1` 补建名均不存在。原 UNKNOWN、attempt、请求摘要和回读证据不改写。
- 四条补建只改变广告名称，账户、父广告组、视频/封面、身份、文案、链接和 CTA 沿用原冻结意图；每条只发送一次。TikTok 四次均返回 `code=0 / message=OK`，新广告 ID 为 `1876737635938434`、`1876737651105874`、`1876737668861154`、`1876737683185906`。应用随后按 ID 回读广告和原组，四条广告与四个原组均为 `ENABLE` 且配置匹配，并写入四条不可变 `build_verified_replacement`。
- 最终业务投影通过 `PASS_FOUR_AD_REISSUE`：R627、6X3F 均为 `COMPLETED`，各自计划/成功广告 `130/130`，失败、未知、待处理、可重试和可核查均为 0，各含 2 条已核实补建。四个 Redis 队列为 0；六服务 active、`NRestarts=0`，操作窗口未发现 Traceback、WorkerLost、OOM、ERROR 或 CRITICAL。
- 原四次失败只保存了 `40002 × 2`、`51002 × 2` 和平台 request ID，旧实现有意丢弃业务 `message`，因此不能事后声称具体字段错误。本次相同业务配置仅改名即全部成功，证明当前素材、封面、身份和父组不存在持续性校验阻断；更符合一次性平台业务异常或名称侧瞬时状态，而不是可稳定复现的参数错误。后续应将经过脱敏的业务 message 纳入私有诊断证据，并把明确业务拒绝与无响应超时分开分类。
