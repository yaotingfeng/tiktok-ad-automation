...
An AD can wait beyond the material evidence age. Its preparation therefore rechecks both the current target video and its cover before compiling the request. An expired READY cover schedules a known-ID GET; a changed VID has a separate cover identity. After admission, the final short arming transaction compares the frozen group's current video/image IDs and evidence ages against the compiled request. Changed evidence leaves the unsent AD pending, with no persisted send intent. Completed MATERIAL and advertising steps are not reopened.
