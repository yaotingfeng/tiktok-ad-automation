...
- [x] 资源池两个实际子进程、结果/搭建独立；Linux27项与恢复链路执行通过，硬期限和共享配额未变。
- [x] 根据2核/2GB主机内存与既有endpoint并发2选择素材准备2；新增slot不扩大外部配额。原批次受UNKNOWN限制，不能据此宣称已测得满负荷吞吐或理论最大值。
- [x] 代码审查、相关回归、静态检查，聚焦提交并推送。
- [x] 完整备份及独立恢复验证后部署，核对六服务实际版本/配置并监控原批次；记录真实结果和未完成项。

## 进度

### 发布后补齐：有界核查

- [x] 单条及批量核查共用整轮预算、stopped 栅栏；仅正式只读入口重开。
- [x] UNKNOWN 原因同步不更改结果，拒权分页不饥饿，前端停止原因可读。
- [x] 最终回归及复审后再次备份、部署，核对新前端和后台版本。
...
- `ee23a5b` 已推送部署，Linux27项、完整备份恢复和六服务十一进程验收通过。原批次94/180、AD189/360，历史13项转存及少量共享/封面/广告UNKNOWN仍需核实；重构与部署完成不等于原批次完成。
- 后续预算补丁 `a6e0b67` 已推送部署；Linux31项、013952Z完整备份恢复、六服务十一进程和新前端一致性通过。01:40UTC旧批次仍94/180、AD189/360，15项扫描已完整结束仍未知，待投递为空；没有重传未知操作。完整回归与最后精选结果见同一验收记录。
