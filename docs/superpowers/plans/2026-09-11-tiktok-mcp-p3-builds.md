...
扩充完整场景：无 App 的 MCP 授权后账户目录→场景→R2 源入库→跨账户目标 VID/封面→预览→CTA→三级→父子/金额/ENABLE 回读。目标素材依赖未完成不得提交。每调用计数、调用 route、实际来源/目标 ID 均断言；租户隔离、共享桶、outbox 去重、refresh 语义、schema 漂移及 R2 未知保护覆盖 P1/P2 对应测试，报告引用具体测试路径。
