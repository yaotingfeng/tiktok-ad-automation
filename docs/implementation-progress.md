## 2026-09-16：新加坡跨 BC 素材库发布准备

- 用户已授权提交推送全部本地应用改动并部署新加坡测试服，来源 BC `7678608005688066065`、目标 BC `7683817908149272592`；真实搭建留给用户手动测试。`9d9f913` 及此前账户授权界面/租户素材库提交已推送 origin/main。
- 新代码只读检查真实 A 的 546 份素材及 B 的 30 个账户全部可准备；3 个实际源视频的规格、MD5/大小和白名单域名核实成功，B 候选主账户真实读取通过。
- Linux 首轮 28 通过/1 失败，修正旧硬终止测试被 1 秒软期限提前中断及旧准入键断言，独立复验 1 通过；仅测试调整。最终回归、备份迁移和发布验收续记于[发布记录](validation/2026-09-16-staging-tenant-library.md)。
