...
- 聚焦提交：`builds: simplify task details and make submitted previews read-only`。仅本地实现，未推送/部署，线上页面尚未更新。
...
- 前端仅在名称单独变化时显示「保存名称」，配置变化仍显示「保存为新版本」并沿用原校验、冲突与未知结果处理。同步重新生成 OpenAPI 客户端类型；无数据库迁移、新开关、TikTok 请求、推送或部署。
