...
- 本轮测试服仍为上一已发布版本，未重复创建真实广告。发布前完整备份、PG/Redis/源码前端/配置恢复，Linux实际队列/满批验收，再迁移切换和导入此前两条补建；最终结论续补于[本轮验收](validation/2026-09-15-build-batching.md)。未推送。
- 首次Linux满批门禁拦截`5ec1c69`，未升级在线库/切版，已恢复旧站点。实机证明每callback885次重复SELECT耗尽40秒预算；事务内source合法映射批读及完整route/BC/account/capability去重降至252次，保留逐成员fence/新鲜度且不跨请求缓存，所有生产时间上限不变。46项批次回归通过（40+6）、根官方传输/素材适配器123通过1跳过；第二轮发布将先跑重型隔离验收，通过才停写备份。
