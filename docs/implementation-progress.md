## 2026-09-17：单条素材失败隔离与预算显示（本地，未发布）
...
- 最终后端整合 122 项、前端准备/预览 106 项通过，预算与策略另 79 项通过（有重叠）；构建、ty、改动 Ruff/Biome、Alembic check 通过。详见[验证及发布边界](validation/2026-09-17-preview-partial-materials.md)。尚未推送或部署，上线需迁移并由用户重新生成预览。
