## 2026-09-16：素材集中上传与封面共享（本地完成，测试服待发布）
...
- 当前线上仍为 `ca69cc3`，尚未切换服务或迁移线上数据。原上传批次 477 available/7 failed，无素材操作在途；不重建、不处理失败视频。主机内存约 2 GiB 且 swap 约 1 GiB 已用，保留现有两个资源槽及配额，不盲增 Worker/QPS。发布须隔离未部署分页功能、完整备份与恢复演练后进行。
