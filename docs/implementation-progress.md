...
- 新增两个真实PG并发回归：跨批次等待低ID时不能持有高ID，以及共享收口等待素材时不能持有批次。正确复现2 failed后修复为2 passed（3.72秒）；批量共享/双通道/未知结果/并发及广告锁序56项通过（180.80秒），Ruff、格式、语法和ty app通过。尚未部署此改动。
