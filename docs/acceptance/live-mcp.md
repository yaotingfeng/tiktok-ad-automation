...
执行前核对真实授权 scope、主体、已绑定 BC、完整账户目录及操作角色、当前接口版本、工程容量和调度租约。上传请求关闭自动修复/绑定，正常业务成功且返回有效 VID 时保存回执并直接完成入库，不逐条重复查询。只有响应丢失、缺少有效 VID 或本地发布中断等未知情况，才保留原件并沿原账户核查；恢复仍严格比较身份和实际摘要。按 [成功回执决策](../superpowers/specs/2026-09-13-upload-success-receipt.md)区分本地原件摘要、上传成功证据与查询证据。
...
2. 账户权限及请求检查通过后，按用户指定原件执行来源上传及目标跨账户准备，记录真实源/目标事实。正常 URL 上传以业务成功和有效 VID 释放用途；来源于未知上传恢复的候选 VID 仍须严格核查，不能以缺少摘要的候选结果释放用途。目标跨账户准备单独验收。
