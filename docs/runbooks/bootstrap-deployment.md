...
Linux 首次部署在编写 systemd/Compose Worker 并发前，必须执行[首次部署的 Worker 容量评估](deployment.md#首次部署的-worker-容量评估)。先测主机、数据库、Redis、API、调用额度和单槽峰值，再分别确定 Resource、Result、Build、Control；仅 Beat 固定为 1。环境手册必须记录测量证据、最终值和回退值，不能复制新加坡测试机的 `2/1/1/1`。
