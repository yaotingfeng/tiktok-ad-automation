...
- 运行提交：`ca69cc3c7239f9440d4091f81a5b68c8337aa810`；已取消已确认账户的 24 小时年龄门槛，租户/全局 R2 原件预算均为 `0`（不限量），入库/清理保持开启，见[权限与原件验收](../validation/2026-09-15-permissions-storage.md)。本版本基于原线上 `4230f8f` 单独应用本次修复，未包含另一个分页功能。广告命名改用版权方数字剧目 ID 见[命名发布验收](../validation/2026-09-15-staging-numeric-drama-id-naming.md)；任务概览收拢与连接误报修正见[任务详情验收](../validation/2026-09-15-task-detail-refinement.md)；策略名称编辑和任务详情只读优化见[策略发布验收](../validation/2026-09-15-staging-strategy-task-ui-release.md)；20×10批量共享、MID/封面批核验、独立广告Worker与原任务补建闭环见[批处理发布验收](../validation/2026-09-15-build-batching.md)。数据库 head 为 `preview_display_drama_id`，旧版本及发布前备份保留；更早发布记录从上述文档继续追溯。
