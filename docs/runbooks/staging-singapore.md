...
> **环境口径（2026-09-19）**：本服务器是测试服务器，也是当前唯一已部署的远程环境；它不是生产环境。当前生产服务器尚未提供，仓库中的旧生产地址不得使用。测试服最新运行代码版本为 `1e9ca7a221988d172e8f99f75d43b552e5a87c7f`，数据库 head 为 `draft_scene_phase`，发布证据见[MCP 广告业务拒绝有限重试验收](../validation/2026-09-19-mcp-ad-business-rejection-retry.md)。以下按日期保留的其他“当前运行”表述均是各次发布时的历史快照。

- 2026-09-19 当前运行 `1e9ca7a221988d172e8f99f75d43b552e5a87c7f`：仅对 MCP 广告创建的已验证明确拒绝码 40002/51002 使用原冻结请求有限重试，其他非零码和未知结果保持禁止重放。有效完整备份 `20260919T065212Z` 的 PostgreSQL、Redis、1,319 个项目/前端文件和私有配置已通过隔离恢复；22 张表、2,705 份加密响应、六服务实际版本、4 Worker、HTTPS、配置与服务器 12 项回归通过。发布未调用产品恢复或 TikTok 写接口，四队列为 0。详见[本轮记录](../validation/2026-09-19-mcp-ad-business-rejection-retry.md)。以下版本记录均为历史。

- 2026-09-19 当前运行 `815161166f956e6775cb1543224cfa8f80d64493`：租户管理员可创建并绑定普通账号；确定终态的素材/封面依赖直接落定并减少重复唤醒；请求正文登记前的本地截止时间采用既有三次有限重试。完整备份 `20260918T191556Z` 的 PostgreSQL、Redis、1,316 个项目/前端文件和私有配置/证书已通过校验及隔离恢复；22 张表、2,060 份加密响应、六服务、4 Worker、HTTPS、配置与调用策略通过。发布未触发广告恢复或重发，R627/6X3F 仍为 NEEDS_REVIEW；四条已发送结果不明的广告须另行明确接受重复风险。详见[本轮记录](../validation/2026-09-19-build-dependency-efficiency-release.md)。以下版本记录均为历史。

- 2026-09-19 当前运行 `f0e089da72eb11738095c93b20a2c70f217fdfb0`：未发送的TikTok本地期限错误进入既有三次有界重试，封面批量同伴竞争仅延迟重排锚点。完整备份 `20260918T170055Z` 的PG/Redis/项目/私密配置与证书均通过校验及隔离恢复；六服务、四Worker、HTTPS和原配置通过。新骏伯R627/6X3F正式RETRY完成14/30项调度，CTA真实创建成功，17项封面领取竞争退出失败态；17:22UTC builds队列323→27但批次仍未整批完成。吞吐与内存边界见[本轮记录](../validation/2026-09-19-cta-cover-contention-release.md)。以下版本记录均为历史。

- 2026-09-18 当前运行 `6f610779d9cb4c6d16e03683b667fcc3c8081013`：批量视频共享按锚点索引边固定为最多20×10候选，并发领取可安全接续下一批；任务列表状态投影与总数改为单条SQL；已结算且无活动所有者的构建Outbox保留七天后压缩正文而不删除幂等/审计身份。`144836Z` 五份归档、1,396个项目文件、22表/1,845份加密响应独立恢复及迁移演练通过，head 为 `outbox_payload_compaction`；六服务、4 Worker、HTTPS、原配置及原批次180/180/360业务投影通过，观察期到期Outbox为0且无重试。按用户要求删除旧备份，仅保留`20260918T144836Z`。详见[本轮验收](../validation/2026-09-18-batch-sharing-hardening.md)。以下版本记录均为历史。

- 2026-09-18 当前运行 `d391dec046eef7ab1660fba3fda1a2354bf68ae9`：明确接受重复广告风险后，允许原组含成功兄弟广告的单条 UNKNOWN 同组补建；原 UNKNOWN 和请求证据不改写，只有新广告、原组和状态真实回读全部匹配才计入完成。`132252Z` 五归档、1,395 项目文件、22 表/1,767 加密响应独立恢复及迁移演练通过，head 为 `ad_same_group_reissue`；六服务、4 Worker 节点、HTTPS 和原配置通过。新骏伯原批次补建广告 `1876676311288962` 已回读为 ENABLE，最终 Campaign/Ad Group/Ad 为 180/180、180/180、360/360，恢复候选为 0。按用户要求删除旧备份，仅保留 `20260918T132252Z`。详见[本轮验收](../validation/2026-09-18-same-group-ad-reissue.md)。以下版本记录均为历史。

- 2026-09-18 最新运行 `bbe3c97d841e3310c76c23ad4518ed26d27e2f22`：成功视频回执不重复读取，封面复用已保存MID且可选MID不阻挡源上传成功，新广告图取消自动READBACK；历史确定成功的核查保留事实但退出调度/统计。Linux28个关键用例通过（180容量用例中断，非通过），074829Z五归档/1388文件/22表/1486响应独立恢复，六服务11进程、队列/原配置/前端/HTTPS通过；head和额度不变。正式RETRY完成10项，07:51UTC完整128/180、AD257/360，仍有真实封面未知/失败和1个原未知AD，不是整批完成。见[本轮验收](../validation/2026-09-18-success-receipt-direct-use.md)。下述旧版当前/最新均为历史记录。
...
- 2026-09-18 当前运行 `b069408d41bdc2f59f50e288c250ec388d13ae10`：跨BC就绪依赖按账户短事务保存进度、同BC共享候选先筛后分页、共享领取有界锁等待、明确失败素材接入原RETRY、SDK错误码保留。Linux90项通过，035747Z五归档/1363文件/21表/1364响应独立恢复、六服务十一进程/私有配置/前端/HTTPS/双身份隔离通过。开关、额度、并发与head不变；03:59UTC仍94/180、AD189/360，17项素材及1广告历史未知未补发，非整批成功，见[本轮验收](../validation/2026-09-18-pipeline-completion.md)。

- 2026-09-18 先前运行 `c0a318f2b61634cb6b1df5b4fe877077ef2f3be3`：目标URL转存及目标核验复用加密响应归档，不改变未知结果重发限制。Linux25项、023751Z五归档/1362文件/21表/1292响应独立恢复、六服务十一进程/私有配置/前端/HTTPS/双身份隔离通过；开关、额度、并发及head不变。02:38UTC原批次94/180、AD189/360，历史未知补发风险待用户选择，非整批成功，见[此前验收](../validation/2026-09-18-target-transfer-receipts.md)。以下为历史发布。

- 2026-09-18 先前运行 `a6e0b67e7f9f0df3e73d540c09b8ac49506110c2`：在统一20×10滚动准备、持久成功回执和seed事件唤醒基础上，单条/批量核查共同使用整轮预算及停止栅栏，明确负详情失效旧映射，前端显示停止原因。资源准备池2，其余结果/搭建/控制各1，六服务十一进程。完整备份 `20260918T013952Z` 和独立恢复、Linux31项、配置/新前端/HTTPS验收通过。历史未知结果仍须核实，不代表整批已成功，见[验收记录](../validation/2026-09-18-material-pipeline-refactor.md)。以下版本条目为历史。
...
- 2026-09-18 当前运行 `ecf233da264efb6962e982abc43f5a5b2244c3be`：原时间/四次读取预算内连续完成首次共享目标查询，正证据立即发布。备份194115Z五归档/1262文件/21表/1033响应恢复一致；六服务十进程/双身份隔离、服务器3项33.28秒通过，测试角色恢复。原配置/额度/head/前端不变；19:44UTC完整4/180、AD8/360，持续验收未完成。私有发布脚本取最新备份需完整消费目录列表（例如`sed -n '1p'`），不能在pipefail下用可能触发SIGPIPE的`head`管道。以下为历史记录。
- 2026-09-18 当前运行 `561471877fd5c8251c6f5ac4c8369a09b67a48d8`：SOURCE未知封面重叠分页补齐。备份192632Z五归档/1261文件/21表/1027响应恢复一致；六服务十进程/双身份隔离、服务器3项22.84秒通过。原配置/额度/head/前端不变；19:29UTC仍2/180、MATERIAL459，不宣称整批完成。以下为历史记录。
- 2026-09-18 当前运行 `ff5c158146826c01737b3e23986434a76c273fb4`：首次共享限定源MID完整查询、未知/已复用保持只读。备份191610Z五归档/1260文件/21表/1023响应恢复一致；六服务十进程/双身份隔离、服务器4项32.07秒通过。原配置/额度/head/前端不变；19:23UTC仍2/180、MATERIAL418，不宣称整批完成。以下为历史记录。
- 2026-09-18 当前运行 `aa01610f1de3f79343d8f78c9058a0706ca40076`：同目标连续最多4页，预算不足持久让出，跨页重叠由完整扫描器核对。备份 `20260917T184707Z` 五归档/1259文件/21表/1007响应独立恢复一致；六服务十进程/双身份隔离、服务器5项回归21.67秒通过，测试角色恢复。配置/额度/head/前端不变；18:51UTC仍2/180、MATERIAL249，仍非业务验收成功。以下为历史记录。
- `b9d6982f08cc6be6da90fd6d4e9a5fca8375c1b3`：共享后/来源自身目标MID正向回读。备份 `20260917T183303Z` 五归档/1259文件/21表/998响应独立恢复一致；六服务十进程/双身份隔离及服务器5项回归23.34秒通过，测试角色恢复。配置/额度/head/前端不变；18:35UTC仍2/180、MATERIAL239、BUILD封面509 READY，不能称整批完成。
