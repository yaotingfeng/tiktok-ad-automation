...
> **环境口径（2026-09-19）**：本服务器是测试服务器，也是当前唯一已部署的远程环境；它不是生产环境。当前生产服务器尚未提供，仓库中的旧生产地址不得使用。测试服最新运行代码版本为 `f0e089da72eb11738095c93b20a2c70f217fdfb0`，数据库 head 为 `draft_scene_phase`，发布证据见[CTA与封面竞争修复验收](../validation/2026-09-19-cta-cover-contention-release.md)。以下按日期保留的其他“当前运行”表述均是各次发布时的历史快照。
