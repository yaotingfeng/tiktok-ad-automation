...
- 本版本数据库 head 为 `material_source_bc`：来源的 BC/素材外键与来源冻结路由独立于目标；旧记录按原约束回填明确关系。备份、排空、迁移后再切版本。存在跨 BC 来源记录时，旧约束不能直接恢复，禁止强行 downgrade。
...
- MCP 工具合同增加共享接口后，旧连接需用已有授权重新读取工具目录。管理员在「账户与授权」的 MCP 连接管理中刷新可见 BC，再同步已绑定 BC；后台对应 `connection_business_centers(refresh=True)` 及既有 BC 同步流程。真实工具校验通过后才更新连接合同版本，无需重新登录授权。目录同步完成后还须执行既有账户角色重检（`start_capability_refresh`／页面重检流程），等待完整角色证据发布；同步期间权限会先归为 UNKNOWN，不能通过手工修改权限位跳过。旧冻结请求保留旧版本，未完成任务需按原状态恢复/重新准备，禁止改写已发送请求。
- 管理员配置入口：「版权方连接」→「查看已发现应用」→ 对应应用的「配置 Mini」。填写该应用实际 TikTok Mini ID 并保存；普通操作员不可修改。关联以租户/版权方连接/应用为范围；同步版权方目录不会清空人工配置。后续搭建检查目标账户可用 Mini，不要求每部剧重复配置。不把版权方包名作为 Mini ID。
