...
- MCP 工具合同增加共享接口后，旧连接需用已有授权重新读取工具目录。管理员在「账户与授权」的 MCP 连接管理中刷新可见 BC，再同步已绑定 BC；后台对应 `connection_business_centers(refresh=True)` 及既有 BC 同步流程。真实工具校验通过后才更新连接合同版本，无需重新登录授权。旧冻结请求保留旧版本，未完成任务需按原状态恢复/重新准备，禁止改写已发送请求。
