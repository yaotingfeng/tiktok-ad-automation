...
首次 MCP 绑定通过完整目录验证后，如果该 BC 尚无默认路由，发布事务会将当前连接初始化为默认；已有默认不会被覆盖。上述“没有默认时要求选择”指运行时选路规则，不能理解为首次绑定不会初始化默认。授权回调与 BC 绑定分开：登录用户可能可见多个 BC，每条 MCP 连接只绑定管理员确认的一个 BC；当前单个 BC 也需要在界面选择后提交绑定。
