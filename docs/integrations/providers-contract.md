...
GET `/api/distribute_admin/drama/list` sends explicit app, title, page, page_size=20. It normalizes id/title/lang and retains int_id as display_drama_id; id remains the opaque external identity for provider calls, while int_id is the numeric identity shown to operators. Search continues past a short nonempty page until an empty page; malformed/non-array data is rejected. This search completeness does not authorize link creation.
