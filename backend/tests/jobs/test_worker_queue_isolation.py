...
def deployment_workers():
    ...
    for name in ("builds", "results"):
        independent = root / f"deploy/staging-{name}.service"
...
@pytest.mark.parametrize("destination", ["builds", "resource-results"])
def test_ready_ad_executes_while_all_material_slots_are_blocked(
    redis_client, pool, destination
):
    ...
    try:
        ...
        if destination == "resource-results":
            # 万条准备积压不能挤占结果槽；复用真正的Celery发布连接。
        ...
        app.send_task("test.isolation.ad", queue=prefix + destination)
