...
TASK_NAME = "builds.generate_preview"
register_dispatch_task(TASK_NAME, "builds")
REPAIR_SECONDS = 120


def queue_preview(session: Session, prep: BuildPreview, *, delay: int = 0) -> None:
    due_at = datetime.now(UTC) + timedelta(seconds=delay)
    prep.repair_after = due_at + timedelta(seconds=REPAIR_SECONDS)
    prep.dispatch_id = enqueue_after_commit(
        ...
        task_name=TASK_NAME,
        task_key=f"build-preview:{prep.id}:{prep.generation}",
        payload={"preview_id": str(prep.id), "generation": prep.generation},
    )
    dispatch = session.get(PendingDispatch, prep.dispatch_id)
    assert dispatch
    dispatch.available_at = due_at
...
def process_preview(
    *, database_engine: Any, tenant_id: UUID, actor_id: UUID, payload: dict[str, Any]
    ...
    from app.modules.builds.previews import continue_preview

    if (
        set(payload) != {"preview_id", "generation"}
        or type(payload["generation"]) is not int
        or payload["generation"] < 0
    ):
        raise DomainError("dispatch_payload_invalid", "预览生成任务参数无效")
    try:
        identity = UUID(payload["preview_id"])
    except ValueError, TypeError, AttributeError:
        raise DomainError("dispatch_payload_invalid", "预览生成任务标识无效") from None
    context = TenantContext(tenant_id=tenant_id, actor_id=actor_id, role="operator")
    with Session(database_engine) as session, session.begin():
        prep = session.exec(
            select(BuildPreview).where(
                BuildPreview.tenant_id == tenant_id, BuildPreview.id == identity
            )
        ).one_or_none()
        if prep is None or prep.actor_id != actor_id:
            raise DomainError("preview_not_found", "预览生成任务不存在")
        ...
        prep = session.exec(
            select(BuildPreview)
            .where(BuildPreview.id == identity)
            .with_for_update()
            .execution_options(populate_existing=True)
        ).one()
        if prep.status != "BUILDING" or prep.generation != payload["generation"]:
            return
        ...
        try:
            with session.begin_nested():
                done = continue_preview(session, context=context, preview_id=identity)
        except DomainError as error:
            prep.status, prep.error_code = "FAILED", error.code
            session.add(prep)
            return
        if not done:
            prep.generation += 1
...
def repair_previews(*, database_engine: Any, limit: int = 100) -> int:
    ...
    with Session(database_engine) as session, session.begin():
        rows = session.exec(
            select(BuildPreview)
            .where(
                BuildPreview.status == "BUILDING",
                BuildPreview.repair_after <= now,
            )
            .order_by(col(BuildPreview.repair_after), col(BuildPreview.id))
            .limit(limit)
        ...
        for prep in rows:
            ...
            if dispatch is None:
                queue_preview(session, prep, delay=0)
            elif (
                dispatch.tenant_id != prep.tenant_id
                or dispatch.actor_id != prep.actor_id
                or dispatch.task_name != TASK_NAME
                or dispatch.payload
                != {"preview_id": str(prep.id), "generation": prep.generation}
            ):
                prep.status, prep.error_code = "FAILED", "dispatch_payload_invalid"
...
@celery_app.task(name="builds.repair_previews")  # type: ignore[untyped-decorator]
def repair_previews_task() -> int:
    return repair_previews(database_engine=engine)
